...
| L1 | VER-QST-003 | REQ-QST-003, IF-QST-002 | `cargo run --bin quest -- status` | Verify current runtime state can be inspected. | Status output names campaign/session state. |
| L1 | VER-QST-004 | REQ-QST-003, IF-QST-002 | `cargo run --bin quest -- roll 1d20+5 --seed S07-scene6 --adv --bless` | Verify deterministic dice command behavior. | Deterministic roll output. |
