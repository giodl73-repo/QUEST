...
| WP-QST-002 | GAP-QST-002, REV-QST-002 | Prove checkpoint/recovery path | Added invalid checkpoint rejection coverage; `cargo test --quiet checkpoint` passes resume and invalid-inbound-state proof. | VER-QST-005, VAL-QST-003 | complete |
...
| WP-QST-004 | TR-QST-001 | Lock module binding prep gate | Binder ordering and map handling were fixed with regression coverage; `cargo run --quiet --bin quest -- bind-module 0007-the-silver-ingot-confession` now regenerates a reviewed seven-scene module with the map outside scene count, and the QUEST CLI load smoke passed. | VER-QST-001, VAL-QST-001 | complete |
